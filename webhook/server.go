...
//	}
//
//	verified := VerifyAPIKey(config.SlotClosedWebhook) // FIXME: apikey here
//	if !verified {
//		log.Println(err)
...
//	}
//
//	err = ioutil.WriteFile("webhook_configs/"+config.SlotClosedWebhook+".json", marshal, 0644) //fixme: apikey here
//	if err != nil {
//		log.Println(err)
//		w.WriteHeader(http.StatusInternalServerError)
//		_, err = w.Write([]byte(`{"message": "internal server error"}`))
